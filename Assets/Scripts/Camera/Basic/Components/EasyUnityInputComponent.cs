...
namespace AdvancedUtilities.Cameras.Components
{
    ...
    public class EasyUnityInputComponent : CameraComponent
    {
        ...
        public bool EnableRotationOnlyWhenMousePressed = false;
        ...
        public bool EnableRotationOnlyWhenCursorLocked = false;
        ...
        public Func<float> m_GetHorizontalFunc;
        public Func<float> m_GetVerticalFunc;
        public Func<float> m_GetZoomInFunc;
        public Func<float> m_GetZoomOutFunc;
        ...
        private InputValues GetEasyUnityInput()
        {
            ...
            float? horizontal = m_GetHorizontalFunc!=null?m_GetHorizontalFunc():Input.GetAxis(HorizontalInputName);
            float? vertical = m_GetVerticalFunc != null ? m_GetVerticalFunc() : Input.GetAxis(VerticalInputName);
#if UNITY_EDITOR
            float? zoomIn = -1f * Input.GetAxis(ZoomInInputName);
            float? zoomOut = -1f * Input.GetAxis(ZoomOutInputName);
#else
            float? zoomIn = m_GetZoomInFunc != null ? m_GetZoomInFunc() : -1f * Input.GetAxis(ZoomInInputName);
            float? zoomOut = m_GetZoomOutFunc != null ? m_GetZoomOutFunc() : -1f * Input.GetAxis(ZoomOutInputName);
#endif

          //  Debug.LogError("========:"+ zoomIn+"_____"+ zoomOut);
        ...
        private void EnforceRotationRestrictions(InputValues input)
        {
            ...
            {
              //  Debug.LogError("Cursor==="+ Cursor.lockState+"  "+Time.frameCount);
                input.Horizontal = null;
                input.Vertical = null;
            }
        }
